...
if($_SERVER['REQUEST_METHOD'] == "POST" && $_FILES['photo_one']['name']!="" && $_FILES['photo_thumb']['name']!="")
{      
	...
	if($a_title!="")
	{
		...
		if($_FILES['photo_thumb']['name']!="")
		{
			...
			$tmpName  = $_FILES['photo_thumb']['tmp_name'];
			$extension = substr(strrchr($_FILES['photo_thumb']['name'], "."), 1);
		...
		if($_FILES['photo_one']['name']!="")
		{
			...
			//$f_name = substr(strrchr($_FILES['photo_one']['name'], "."), 0);
			$newpath= $_FILES['photo_one']['name'];
			$moveto= $goods_id."/".$newpath;
			move_uploaded_file($tmpName,$moveto);
		}	
		...
		$sth = $conn->prepare ("INSERT INTO news (news_head,news_type,news_id,author,credited,no_read,news_date)
														VALUES (?,?,?,?,?,?,now())");																
					$sth->bindValue (1, $a_title);
					$sth->bindValue (2, $type);
					$sth->bindValue (3, $folder_name);
					$sth->bindValue (4, $_SESSION['Admin_user_full_name']);
		...
		<div class="row">
				...
				<div  class="col-sm-12 col-md-12 col-lg-12">
					...
					<div  class="col-sm-12 col-md-12 col-lg-12"  class="col-lg-12" style="width:100%; padding-top:10px; padding-left:0px; padding-bottom:5px; background-color:CadetBlue;margin-bottom:1%;color:yellow">
							...
							<h4>&darr; Upload New Videos &darr;</h4>
							<?php echo $err;?>
						<hr/>
						
						<div class="col-xs-10 col-sm-10">
						<form role="form"   id="form" class="form-horizontal" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data" method="POST">
							...
							<div class="form-group">
									<label for="a_price" class="control-label col-xs-3">Credited To : </label>
									<div class="col-xs-5">
										<div class="input-group">
												<span class="input-group-addon"><span class="glyphicon glyphicon-pencil"></span></span>
							...
							<div class="form-group">
									<label for="photo_one" class="control-label col-xs-3">Video / Audio Thumbnail :<span style="color:red;padding:0px"class"require">*</span></label>
									<div class="col-xs-9">
										<div class="input-group">
												<input  type="file"   id="photo_thumb" value="browse" name="photo_thumb"  ></input>
											</div>
									</div>									
							</div>
							<div class="form-group">
									<label for="photo_one" class="control-label col-xs-3">Video / Audio File :<span style="color:red;padding:0px"class"require">*</span></label>
									<div class="col-xs-9">
										<div class="input-group">
