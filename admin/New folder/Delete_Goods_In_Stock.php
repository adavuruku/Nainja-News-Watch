			...
			<div  class="col-sm-1 col-md-1 col-lg-1"  >
				...
				<div  class="col-sm-10 col-md-10 col-lg-10">
					<div  class="col-lg-12" style="width:100%; padding-top:5px; padding-left:5px; padding-bottom:10px; background-color:grey;margin-bottom:1%">
						<h3 style="text-align:center;color:white">The Profiler - Delete Goods</h3>
					...
					<div  class="col-sm-12 col-md-12 col-lg-12"  class="col-lg-12" style="width:100%;  padding:10px; background-color:CadetBlue;margin-bottom:1%;">
							<?php
											...
											$sql    = "SELECT count(id) FROM store_goods ";
											...
											$status="0";
											$sql = "(SELECT * FROM store_goods where status='".$status."' ORDER BY id ASC LIMIT {$per_page} OFFSET {$offset})"; 
											$retval = mysql_query($sql, $conn_two);
											if (!$retval) {
												die('Could not get data: ' . mysql_error());
											}else{
											...
											$dateprint = "For : ".$J.", ".$Q;	
											echo '<h4 style="text-align:left;color:yellow">List Of All Goods In Stocks - '.$dateprint.'</h4><p id="result2"></p>';
											...
											while ($row_retrieve = mysql_fetch_array($retval, MYSQL_ASSOC)) 
											{
												...
												$data=$data.'<tr >
																	...
																	<td>'.$row_retrieve['product_name'].'</td>
																	<td>'.$Amount_Naira.'</td>
																	<td>'.$row_retrieve['details'].'</td>
																	<td>'.$row_retrieve['category'].'</td>
																	<td>'.$row_retrieve['sub_category'].'</td>
																	<td>'.$row_retrieve['date_register'].'</td>
																	<td><a href="#" title="Delete_Item" style="color:yellow" onclick="delete_Cart(\''.$row_retrieve['product_id'].'\',\''.$row_retrieve['product_name'].'\')"><img src="../store_files/images/delete-icon.jpg" style="height:20px" ></img></a></td>
											...
											if ($total_pages > 1)
											{
												...
												{
												echo ' <li><a href=Delete_Goods_In_Stock.php?page='.$previous_page.'>&laquo; </a> </li>';
												}
												 ...
												 for($i = 1; $i <= $total_pages; $i++)
												 {
													...
													//then it sset the i to be active or focused
													 ...
													 //display the page number
														echo ' <li><a href=Delete_Goods_In_Stock.php?page='.$i.'> '. $i .' </a></li>';
													 }
												 }
												//this is for next record		
												if ($has_next_page)
												{
													echo ' <li><a href=Delete_Goods_In_Stock.php?page='.$next_page.'>&raquo;</a></li> ';
												}
