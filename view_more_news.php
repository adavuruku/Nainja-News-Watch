	...
	<?php require_once 'settings/header_file.php';?>
		...
		<div class="row" >
			<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="background-color:white;">
				<h5 style="border-bottom-right-radius:2em;border-top-right-radius:2em;background:#dddddd;padding: 10px 0px 10px 10px;color:red;"><b>Home &raquo; VIEW MORE OF <span style="color:blue;font-family:Due;"><?php echo strtoupper($news_c);?> NEWS !!</span></b></h5>
				<hr/>
			</div>
			<!-- cont before news -->
			<div class="col-xs-12 col-sm-12 col-md-8 col-lg-8" style="background-color:#eeeeee;">
				...
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" >
					...
					<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="margin-top:10px;background-color:white;">
						<p style="color:red;font-weight:bold;background-color:white;padding:15px 0px 0px 10px;"><strong>MORE <span style="color:red;font-family:Due;"><?php echo strtoupper($news_c);?> NEWS !! </span>&raquo;<strong></p>
						<hr/>
					</div>
					<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" >
					<?php
						if($news_c=="latest"){
							$stmt_ina = $conn->prepare("SELECT * FROM news ORDER BY id DESC Limit 500");
							$stmt_ina->execute();
						}elseif($news_c=="commented"){
							$stmt_ina = $conn->prepare("SELECT * FROM news where category=?");
							$stmt_ina->execute(array($news_c));
						}elseif($news_c=="trending"){
							$stmt_ina = $conn->prepare("SELECT * FROM news order by no_read desc limit 500");
							$stmt_ina->execute();
						}elseif($news_c=="archives"){
							$stmt_ina = $conn->prepare("SELECT * FROM news order by id ASC Limit 500");
							$stmt_ina->execute();
						}else{
							$stmt_ina = $conn->prepare("SELECT * FROM news where category=? limit 500");
							$stmt_ina->execute(array($news_c));
						}
						...
						$has_next_page = $next_page <= $total_pages ? true : false;
							...
							//start
							if($news_c=="latest"){
								$stmt_in = $conn->prepare("SELECT * FROM news ORDER BY id DESC Limit {$per_page} OFFSET {$offset} ");
								$stmt_in->execute();
							}elseif($news_c=="commented"){
								$stmt_in = $conn->prepare("SELECT * FROM news where category=?");
								$stmt_in->execute(array($news_c));
							}elseif($news_c=="trending"){
								$stmt_in = $conn->prepare("SELECT * FROM news ORDER BY no_read DESC Limit {$per_page} OFFSET {$offset} ");
								$stmt_in->execute();
							}elseif($news_c=="archives"){
								$stmt_in = $conn->prepare("SELECT * FROM news order by id ASC Limit {$per_page} OFFSET {$offset} ");
								$stmt_in->execute();
							}else{
								$stmt_in = $conn->prepare("SELECT * FROM news where category =? ORDER BY id DESC Limit {$per_page} OFFSET {$offset} ");
								$stmt_in->execute(array($news_c));
							}
								...
								<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">	
									<ul class="pagination" align="center">';									
										if ($total_pages > 1)
										{
											...
											{
												echo ' <li><a href=view_more_news.php?page='.$previous_page.'&news_c='.$news_c.'>&laquo; </a> </li>';
											}
											 ...
											 for($i = 1; $i <= $total_pages; $i++)
											 {
												...
												//then it sset the i to be active or focused
												 ...
												 //display the page number
													echo ' <li><a href=view_more_news.php?page='.$i.'&news_c='.$news_c.'> '. $i .' </a></li>';
												 }
											 }
											//this is for next record		
											if ($has_next_page)
											{
											echo ' <li><a href=view_more_news.php?page='.$next_page.'&news_c='.$news_c.'>&raquo;</a></li> ';
											}
